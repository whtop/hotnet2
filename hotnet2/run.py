...
def run_helper(args, infmat, full_index2gene, G, nname, pnp, heat, hname, 
               addtl_genes, get_deltas_fn, infmat_name="PPR", 
               max_cc_sizes=[5, 10, 15, 20], verbose=0):
    ...
    if args.deltas:
        ...
        deltas = get_deltas_fn(full_index2gene, heat, args.network_permutations,
                               args.num_cores, infmat, addtl_genes, pnp, infmat_name, 
...
def get_deltas_hotnet2(full_index2gene, heat, num_perms, num_cores, _infmat, 
                       _addtl_genes, permuted_networks_path, infmat_name, 
                       max_cc_sizes, verbose):
    # find smallest delta
    deltas = get_deltas_for_network(permuted_networks_path, heat, infmat_name,
                                    full_index2gene, MAX_CC_SIZE, max_cc_sizes,
                                    False, num_perms, num_cores, verbose)

    # and run HotNet with the median delta for each size
    return [np.median(deltas[size]) for size in deltas]

def get_deltas_classic(full_index2gene, heat, num_perms, num_cores, infmat, 
                       addtl_genes, min_cc_size, max_cc_size, verbose):
    # find delta that maximizes # CCs of size >= min_cc_size for each permuted data set
    deltas = get_deltas_for_heat(infmat, full_index2gene, heat, addtl_genes, 
                                 num_perms, NUM_CCS, [min_cc_size], True, 
                                 num_cores, verbose)
